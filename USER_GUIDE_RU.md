...
Меняй вопросы и ответы прямо в `ticket.docx`. Команда:

```cmd
ist-ticket 1
```

выводит текст из этого Word-файла. Таблицы из `ticket.docx` выводятся в консоль как обычные текстовые таблицы.

Если `ticket.docx` в папке нет, библиотека берет запасной файл:
...
В каждой папке может быть `ticket.docx`. Это основной Word-файл билета для команды `ist-ticket`.
Если `ticket.docx` нет, используется запасной `ticket.md`.
